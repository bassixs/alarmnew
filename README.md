...
Конвейер: **сбор → фильтрация → выбор редактора → рерайт → выбор визуала → утверждение → публикация**.

Актуальное состояние, реализованный редакционный процесс и план улучшения качества:
[docs/STATUS_AND_ROADMAP.md](docs/STATUS_AND_ROADMAP.md).
Обновление от 25 сентября 2026: [доставка карточек, лимиты и история решений](docs/RELIABILITY.md).
...
| Модерация | `aialarm.moderation` | MAX/Telegram, выбор новости и визуала, журнал решений, повторная доставка карточек |
...
- `sensitive_only` — вручную чувствительные новости, прошедшие фильтр, остальное автоматически;
- `off` — автопубликация (но чувствительное всё равно на модерацию — не роняем молча).

Узнать свой `admin_chat_id`: напишите боту `/start`.

В основном контуре редактор сначала выбирает «Переписать», затем визуал: оригинал,
генерация или без картинки. После правки текста выбор визуала повторяется.
При отмене новости и отклонении поста бот предлагает выбрать причину; она и ID
редактора сохраняются вместе с историей текстов. Районные карточки используют свой
процесс и исходные фотографии.

Ручное утверждение также учитывает дневной лимит и интервал. Если публикация пока
невозможна, пост остаётся в очереди; выбранные площадки и контур сохраняются.

### Сменный режим помощника

`duty_schedule` задаёт рабочие окна в локальном часовом поясе. Вне смены MAX-бот
остаётся запущенным для управления, но сбор, фильтрация и плановая публикация
приостанавливаются. При первом сборе после начала смены принимаются только новости,
опубликованные после её начала — дневная лента редакторов не превращается в бэклог.

Панель в MAX открывается командами `/bot` или `/status`:

- `▶️ Включить` / `/bot_on` — ручной ON до ближайшей границы расписания;
- `⏸ Выключить` / `/bot_off` — ручной OFF до ближайшей границы;
- `🕒 AUTO` / `/bot_auto` — немедленный возврат к расписанию;
- `📊 Статус` — текущее состояние и следующее переключение.

Если заполнен `moderation.control_user_ids`, управлять режимом смогут только
перечисленные MAX user ID. При пустом списке панель доступна участникам закрытого
чата модерации.
...
- **картинки**: реализована загрузка через `POST /uploads` с последующим добавлением
  полученного payload в attachments; основной контур поддерживает до 10 фотографий;
